...
percentage.added <- function(i.data, i.n) {
  ...
  return(data.frame(percentage = per, start = rs.max, end = rs.max + n - 1, n = n, sum = sum(i.data[rs.max:(rs.max + n - 1)]), max = rs.max - 1 + which.max(i.data[rs.max:(rs.max + n - 1)]), convrate = convrate, stringsAsFactors = FALSE))
}
